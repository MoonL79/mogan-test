...
5. 用结构化命令插入：
   ...
   - 链接
   - 表格
6. 最后检查文档是否出现了原始标记字符串；如果出现，说明排版失败，需要修正。
...
3. 当前默认约定是：矩阵应插入到 displayed formula 中，使其自然居中显示。
...
3. 尽量让文档逐步成形，而不是一次性覆盖成一大段结果。
4. 输出给用户的总结中，说明你“实际做了哪些文档操作”，而不是只贴最终文本。
