...
(define *workflow-formatting-policy*
  "写入 Mogan 时不要把 <with|...>、<math|...>、<matrix|...> 等原始 TeXmacs 标记当作正文文本直接插入；先写纯文本结构，再调用现有命令处理标题、强调、代码、链接、公式、分式、矩阵等结构；只有在使用 `insert-section`、`insert-subsection`、`insert-subsubsection` 时，才需要紧接着调用 `exit-right` 跳出标题结构，并在需要正文段落时再使用 `insert-return` 和 `insert-text`；标题文本本身不要携带显式编号，编号交给环境自动生成；完成前自检文档中不应残留原始 <tag|...> 文本")

(define *workflow-next-step*
  "先用 `xmake b stem` 构建，再用 `mogan-cli start-server` 或等价的 `moganstem -server -x '(load .../mogan-server-runtime.scm)'` 启动可连接的 server，接着用 `mogan-cli target save` 保存 target 配置，然后通过 `target run`、`batch`、`scenario smoke-edit`、`scenario batch-smoke`、`scenario file-smoke`、`scenario export-smoke`、`scenario style-smoke`、`scenario layout-smoke`、`scenario search-smoke`、`scenario history-smoke`、`scenario clipboard-smoke` 驱动 live server；如果插入 `section`、`subsection`、`subsubsection`，先执行 `exit-right` 跳出标题结构，再按需要执行 `insert-return` 和正文写入；section 类标题不要手写编号，交给环境自动编号")
