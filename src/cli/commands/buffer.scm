...
(define *formatting-policy-embedded*
  "写入 Mogan 时不要把 <with|...>、<math|...>、<matrix|...> 等原始 TeXmacs 标记当作正文文本直接插入；先写纯文本结构，再用现有命令处理标题、强调、代码、链接、公式、分式、矩阵等结构；只有在使用 `insert-section`、`insert-subsection`、`insert-subsubsection` 时，才需要紧接着调用 `exit-right` 跳出标题结构，并在需要正文段落时再使用 `insert-return` 和 `insert-text`；标题文本本身不要携带显式编号，编号交给环境自动生成；完成前自检文档中不应残留原始 <tag|...> 文本")
(define *split-layout*
