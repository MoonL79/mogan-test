...
(define (status-data)
  (list
    ...
    (cons "control_surface" "state, move-*, select-*, undo, redo, copy, cut, paste, clear-undo-history, insert-text, delete-*, save-buffer, export-buffer, set-main-style, set-document-language, add-style-package, remove-style-package, switch-buffer")
    ...
    (cons "scenario_style_command" "./mogan-cli scenario style-smoke smoke")
    ...
    (cons "style_control_surface" "set-main-style, set-document-language, add-style-package, remove-style-package")
...
(define (cmd-workflow args)
  (make-success
    (cons "steps"
          "1. cd /home/mingshen/git/mogan 2. xmake b stem 3. Start a connectable server with `mogan-cli start-server` or an equivalent `moganstem -server -x '(load .../mogan-server-runtime.scm)'` 4. Save a target profile with `mogan-cli target save <name>` 5. Run `mogan-cli target run <name> state`, `mogan-cli batch <name> -- new-document -- insert-text ...`, `mogan-cli scenario smoke-edit`, `mogan-cli scenario batch-smoke <name>`, `mogan-cli scenario file-smoke <name>`, `mogan-cli scenario export-smoke <name>`, `mogan-cli scenario style-smoke <name>`, `mogan-cli scenario search-smoke <name>`, `mogan-cli scenario history-smoke <name>`, or `mogan-cli scenario clipboard-smoke <name>` to drive the live server 6. Run `create-account`, `connect`, `write-text`, `buffer-text`, `export-buffer`, `set-main-style`, `set-document-language`, `add-style-package`, `remove-style-package`, `ping`, or `new-document` as needed")
    (cons "constraints"
...
(define (cmd-connect args)
  (make-success
    ...
    (cons "current_result" "The shell wrapper already drives a real controller runtime; login succeeds or fails entirely against the live `-server` instance and supplied credentials")
     (cons "next_step" "If a connectable server is already running with `mogan-server-runtime.scm` loaded, call `./mogan-cli create-account` once for the target credentials and then run `./mogan-cli connect`; add `state`, `move-*`, `select-*`, `undo`, `redo`, `copy`, `cut`, `paste`, `clear-undo-history`, `insert-text`, `delete-*`, `save-buffer`, `export-buffer`, `set-main-style`, `set-document-language`, `add-style-package`, `remove-style-package`, `buffer-list`, `open-file`, `save-as`, `revert-buffer`, `close-buffer`, `search-state`, `search-set`, `search-next`, `search-prev`, `search-first`, `search-last`, `replace-set`, `replace-one`, `replace-all`, `write-text`, `buffer-text`, `batch`, `scenario smoke-edit`, `scenario batch-smoke`, `scenario file-smoke`, `scenario export-smoke`, `scenario style-smoke`, `scenario search-smoke`, `scenario history-smoke`, or `scenario clipboard-smoke` after login succeeds")))
...
(define (show-usage)
  ...
  (newline)
  (display "  set-main-style - Set the current document's main style")
  (newline)
  (display "  set-document-language - Set the current document language")
  (newline)
  (display "  add-style-package - Add a document style package")
  (newline)
  (display "  remove-style-package - Remove a document style package")
  (newline)
  ...
  (newline)
  (display "  scenario style-smoke - Run a target-backed style workflow")
  (newline)
